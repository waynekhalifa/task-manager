...
import {
  ...
  permissionsInput,
} from "framework/permissions/updatePermissions";
...
const PermissionsTable: React.FC<Props> = ({
  ...
  const compareKeysToPermissions = (
    key: string,
  ...
  const handleCheck = (element: string, key: string, value: boolean): void => {
    ...
    const selectedPermission = permissions.find(
      (permission: Permission) => permission.model === element
    );
    if (!selectedPermission) return;
    const permissionKey: string = compareKeysToPermissions(
      key,
      selectedPermission
    );

    // filter based on key and permissionKey
    const selectedPermissionId = permissions.find(
      (permission: Permission) =>
        permission.model === element && permission.name === permissionKey
    )?.id;
    ...
    if (isToRemove) {
      ...
      setAddedPermissions(
        addedPermissions.filter(
          (permissionId: number) => permissionId !== selectedPermissionId
        )
      );
  ...
  const handleTableContent = (element: string): JSX.Element[] => {
    return permissionTypes.map((permissionType, i) => {
      const isChecked =
        checkedState[element]?.[permissionType.key] || isDepartmentAdmin;
      return (
            ...
            onChange={() => {
              const newCheckedState: CheckedState = {
                ...
                [element]: {
                  ...
                  [permissionType.key]:
                    !checkedState[element]?.[permissionType.key],
                },
              };
  ...
  const handleTableRow = (): JSX.Element[] => {
    const mainCategories = [
      ...Array.from(
        new Set(permissions.map((permission: any) => permission.model))
      ),
    ];
