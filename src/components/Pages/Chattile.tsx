...
const Chattile: React.FC<Props> = ({ data }) => {
  ...
  const { activeChatIndex, chatData, txtMessage } = state;
  ...
  const onChangeMessageText = (e: any) =>
    setState((prevState) => ({ ...prevState, txtMessage: e }));
  ...
  const onMessgeSend = () => {
    ...
    setState((prevState) => ({ ...prevState, chatData: [...dd] }));
  };
  ...
  return (
    <div className="col-12 d-flex">
      ...
      <div className="card card-chat-body border-0 order-1 w-100 px-4 px-md-5 py-3 py-md-4">
        ...
        >
          {chatData[activeChatIndex].messages.map((data: any, i: any) => {
            return (
              <li
                key={"messages" + i}
                className={
